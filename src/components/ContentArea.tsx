...
export function ContentArea({ selectedCategory, selectedTopic, language }: ContentAreaProps) {
  if (!selectedCategory) {
    return (
      <main className="flex-1 flex items-center justify-center p-8">
        <div className="text-center max-w-2xl">
          ...
          <h1 className="text-3xl text-foreground mb-4">
            {language === 'vi' ? 'Chào mừng đến IT Interview Prep' : 'Welcome to IT Interview Prep'}
          </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-left">
            <div className="p-4 bg-accent rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">
                {language === 'vi' ? 'Từ Fundamentals đến DevOps' : 'From Fundamentals to DevOps'}
              </p>
            </div>
            <div className="p-4 bg-accent rounded-lg">
              <div className="text-2xl mb-2">💡</div>
              ...
              <p className="text-sm text-muted-foreground">
                {language === 'vi' ? 'Tài liệu được biên soạn kỹ' : 'Carefully curated materials'}
              </p>
            </div>
            <div className="p-4 bg-accent rounded-lg">
              <div className="text-2xl mb-2">🌐</div>
              ...
              <p className="text-sm text-muted-foreground">
                {language === 'vi' ? 'Tiếng Việt & English' : 'Vietnamese & English'}
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 p-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-3xl text-foreground">{selectedCategory.name[language]}</h1>
              <p className="text-muted-foreground">{selectedCategory.description[language]}</p>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            {selectedCategory.topics.map((topic) => (
              ...
              >
                ...
                {topic.subtopics && (
                  <p className="text-sm text-muted-foreground">
                    {topic.subtopics.length} {language === 'vi' ? 'chủ đề con' : 'subtopics'}
                  </p>
                )}
              </div>
            ))}
  ...
  return (
    <main className="flex-1 p-8 overflow-y-auto">
      <article className="max-w-4xl mx-auto prose prose-lg prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-code:text-primary prose-pre:bg-muted prose-a:text-primary prose-li:text-foreground">
        <div dangerouslySetInnerHTML={{ __html: parseMarkdown(selectedTopic.content?.[language] || '') }} />
      </article>
    </main>
  );
}