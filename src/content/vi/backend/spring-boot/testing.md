...
## 2. Unit Testing với Mockito

### 2.1. Mock/Stub/Verify cơ bản
...
// Cú pháp Do-Return (cho void methods)
...
Full application context cho integration tests.
...
| Annotation | Test cái gì | Load những gì |
|-----------|------------|--------------|
| `@WebMvcTest(Controller.class)` | Controller layer | Chỉ web layer (service mocked) |
...
| `@JdbcTest` | JDBC | JDBC trực tiếp, không JPA |

```java
// Test Controller (service được mock)
...
## 5. Testcontainers (Database thật)
...
class UserRepositoryTest {
    ...
    void shouldWorkWithRealDatabase() {
        // Tests chạy trên PostgreSQL thật
    }
}
```
...
// Hamcrest (cũng có trong spring-boot-starter-test)
...
Spring Batch cung cấp `@SpringBatchTest` và `JobLauncherTestUtils` để test batch job toàn diện.
...
### 8.2. @SpringBatchTest va JobLauncherTestUtils
...
### 9.2. @CreatedDate va @LastModifiedDate
...
public class Product {
    ...
    @Version
    private Long version; // cho optimistic locking
